...
export default {
  async fetch(request, env, ctx) {
    ...
    const globalAdminResults = (suspendedGuard || publicReportContract || observation || platformOperator) ? null : await handleGlobalAdminResultsRequest(telegramRequest.clone(), env, ctx);
    const nativeMenu = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults) ? null : await handleTelegramNativeMenuCommand(telegramRequest.clone(), env, ctx);
    const resultGovernance = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu) ? null : await handleResultGovernanceRequest(telegramRequest.clone(), env, ctx);
    const publicResult = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance) ? null : await handlePublicResultRequest(telegramRequest.clone(), env, ctx);
    const dirigentesLifecycle = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance || publicResult) ? null : await handleDirigentesLifecycleRequest(telegramRequest.clone(), env, ctx);
    const portal = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance || publicResult || dirigentesLifecycle) ? null : await handlePortalRequest(telegramRequest.clone(), env, ctx);
    const clubAdminScore = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance || publicResult || dirigentesLifecycle || portal) ? null : await handleClubAdminSeriesScore(telegramRequest.clone(), env, ctx);
    const series = (suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance || publicResult || dirigentesLifecycle || portal || clubAdminScore) ? null : await handleSeriesRequest(telegramRequest.clone(), env, ctx);
    const response = suspendedGuard || publicReportContract || observation || platformOperator || globalAdminResults || nativeMenu || resultGovernance || publicResult || dirigentesLifecycle || portal || clubAdminScore || series || await worker.fetch(request, env, ctx);
