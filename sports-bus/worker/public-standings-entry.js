...
function makeRow(team,championshipCode){
  return {
...
function addVerifiedResult(row,result,side){
  ...
  row.base_points+=pointsForSeries(result.series_code,home,away,side);
...
function h2hPoints(teamId,tiedIds,championshipCode,results){
  let points=0;
  for(const result of results){
    if(championshipForSeries(result.series_code)!==championshipCode) continue;
    if(result.home_id===teamId&&tiedIds.has(result.away_id)){
      points+=pointsForSeries(result.series_code,result.home_score,result.away_score,'HOME');
    }else if(result.away_id===teamId&&tiedIds.has(result.home_id)){
      points+=pointsForSeries(result.series_code,result.home_score,result.away_score,'AWAY');
    }
  }
  return points;
}

function rankRows(rows,championshipCode,results){
  ...
  for(const total of orderedTotals){
    ...
    for(const row of sameTotal){
      row.head_to_head_points=h2hPoints(row.team_id,tiedIds,championshipCode,results);
    }
  ...
  return ranked.map(row=>({
    ...
    championship_code:row.championship_code,
    points:row.points,
    base_points:row.base_points,
    adjustment_points:row.adjustment_points,
    played_matchdays:row._fixture_ids.size,
...
function championshipProjection(container,championshipCode,verified){
  const rule=CHAMPIONSHIP[championshipCode];
  return {
    championship_code:championshipCode,
    label:rule.label,
    independent:rule.independent,
    included_series:[...rule.series],
    max_points_per_matchday:rule.max_points_per_matchday,
    rows:rankRows([...container[championshipCode].values()],championshipCode,verified)
  };
}
...
export function calculatePublicStandings({teams=[],results=[],adjustments=[]}={}){
  ...
  for(const team of teams){
    ...
    groups.get(team.group_id).PRINCIPAL.set(team.team_id,makeRow(team,'PRINCIPAL'));
  ...
  for(const result of verified){
    const championshipCode=championshipForSeries(result.series_code);
    if(!championshipCode||!groups.has(result.group_id)) continue;
    const championship=groups.get(result.group_id)[championshipCode];
    const home=championship.get(result.home_id);
    const away=championship.get(result.away_id);
  ...
  for(const adjustment of adjustments){
    ...
    if(!['PRINCIPAL','SENIOR'].includes(adjustment.championship_code)) continue;
    const team=teamIndex.get(adjustment.team_id);
    if(!team) continue;
    const groupId=adjustment.group_id||team.group_id;
    const row=groups.get(groupId)?.[adjustment.championship_code]?.get(adjustment.team_id);
    if(!row) continue;
  ...
  for(const groupId of [...groups.keys()].sort((a,b)=>String(a).localeCompare(String(b),'es'))){
    ...
    for(const championshipCode of ['PRINCIPAL','SENIOR']){
      for(const row of container[championshipCode].values()){
    ...
    output.push({
      group_id:groupId,
      championships:[
        championshipProjection(container,'PRINCIPAL',verified),
        championshipProjection(container,'SENIOR',verified)
      ]
    });
  }
  ...
  return {
    ok:true,
    contract:'public-standings-v2',
    competition_id:PUBLIC_COMPETITION_ID,
    competition_model:'TWO_INDEPENDENT_CHAMPIONSHIPS_SHARED_MATCHDAY',
    championships:{
      PRINCIPAL:{
        label:CHAMPIONSHIP.PRINCIPAL.label,
        included_series:[...CHAMPIONSHIP.PRINCIPAL.series],
        max_points_per_matchday:CHAMPIONSHIP.PRINCIPAL.max_points_per_matchday
      },
      SENIOR:{
        label:CHAMPIONSHIP.SENIOR.label,
        included_series:[...CHAMPIONSHIP.SENIOR.series],
        max_points_per_matchday:CHAMPIONSHIP.SENIOR.max_points_per_matchday,
        independent_from:'PRINCIPAL'
      }
    },
    rules:{
...
export async function buildPublicStandings(env){
  ...
  const [teamsResult,resultsResult,adjustmentsResult]=await Promise.all([
    ...
    `).bind(PUBLIC_COMPETITION_ID).all(),
    env.DB.prepare(`
      SELECT adjustment_id,competition_id,group_id,championship_code,team_id,points_delta,
