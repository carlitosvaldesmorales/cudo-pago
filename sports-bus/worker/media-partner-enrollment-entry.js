...
export async function handleMediaPartnerEnrollmentRequest(request,env){
  ...
  if(data==='mp:collab:invite'){
    ...
    if(pending){
      ...
      await send(env,chatId,'🕒 Ya existe una invitación pendiente para vincular una identidad de Chépica Play.',{inline_keyboard:[[{text:'🕒 Invitaciones',callback_data:'mp:invites'}],[{text:'⬅️ Chépica Play',callback_data:'mp:manage'}]]});
    ...
    await env.DB.batch([
      ...
      auditStmt(env.DB,`mp-invite-${inviteId}`,actorId,reporter,'CREATE_MEDIA_PARTNER_COLLABORATION_INVITE','partner_collaboration',PARTNER_CODE,1,'two_capability_partner_invite',now.toISOString())
    ]);
    const deepLink=`https://t.me/FutbolChepicaBot?start=partner_${token}`;
    await answer(env,callback.id,'Invitación creada');
    await send(env,chatId,`🤝 VINCULAR · ${PARTNER_NAME}\n\nEsta invitación vincula una identidad al campeonato con exactamente dos capacidades:\n\n1. ⚽ Consultar resultados\n2. 📝 Registrar resultados\n\nNo asigna partidos, coberturas, corresponsales ni eventos en vivo. Tampoco entrega gobierno del campeonato.\nVigencia: ${INVITE_TTL_DAYS} días.\n\n${deepLink}`,{inline_keyboard:[[{text:'🕒 Invitaciones',callback_data:'mp:invites'}],[{text:'⬅️ Chépica Play',callback_data:'mp:manage'}]]});
...
async function showManagement(env,chatId){
  ...
  await send(env,chatId,`🎥 ${PARTNER_NAME}\n\nEstado: ${members.length>0?'✅ ACTIVO':'⚪ SIN IDENTIDAD VINCULADA'}\nIdentidades activas: ${members.length}\nInvitaciones pendientes: ${Number(pending?.n||0)}\n\nCAPACIDADES ACTUALES\n1. ⚽ Consumir resultados\n2. 📝 Registrar resultados\n\nNo incluye coberturas, corresponsales, goles/eventos en vivo ni gobierno del campeonato.`,{inline_keyboard:[
    [{text:'🤝 Vincular identidad',callback_data:'mp:collab:invite'}],
...
async function showMembers(env,chatId){
  ...
  await send(env,chatId,`👥 IDENTIDADES · ${PARTNER_NAME}\n\n${text}\n\nTodas las identidades activas comparten el mismo contrato actual: consultar resultados + registrar resultados.`,{inline_keyboard:rows});
}
