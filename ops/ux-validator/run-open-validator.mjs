...
function deterministicMetrics(code) {
  const hs = code.indexOf('async function showHome');
  const he = code.indexOf('async function showDatePicker');
  ...
  const structuralRedFlag = Boolean(
    messageAccumulationRisk && homeChoiceCount >= 5 && navigationDepthToMatchFromHome >= 3
  );
  return {
    homeChoiceCount,
    usesSendMessage,
...
const reportSchema = {
  ...
  required: ['confidence', 'findings', 'what_to_preserve', 'redesign_principles', 'unknowns'],
  properties: {
...
function deriveJudgement(findings) {
  const severities = (Array.isArray(findings) ? findings : [])
    .map(f => f?.severity)
    .filter(s => severityOrder.includes(s));
  if (!severities.length) return { highest_severity: 'NONE', verdict: 'PASS' };
...
OUTPUT DISCIPLINE:
- findings contains ONLY usability problems. Never put strengths or praise in findings; put strengths in what_to_preserve.
- Return 0 to 2 highest-priority problems. If you find no supported problem, return findings: [].
- Do NOT choose PASS/REJECT/CONDITIONAL and do NOT choose highest severity outside each finding; the deterministic runner owns that decision.
...
const agents = [
  {
    ...
    model: 'qwen2.5:1.5b',
    focus: 'Cognitive walkthrough: decisions, information scent, working memory, depth, backtracking, speed to primary job.'
  },
  {
    name: 'Pixel',
...
function callOllama(payload, agentName) {
  ...
  try {
    execFileSync('curl', [
      ...
      '--data-binary', `@${req}`, '-o', res,
      'http://127.0.0.1:11434/api/generate'
    ], { stdio: 'inherit', timeout: 790000 });
  } catch {
    let b = '';
    try { b = fs.readFileSync(res, 'utf8'); } catch {}
    throw new Error(`${agentName} local Ollama call failed${b ? `: ${b.slice(0, 2000)}` : ''}`);
  }
...
for (const agent of agents) {
  ...
  console.log(`${agent.name} derived verdict: ${r.verdict} (${r.highest_severity})`);
}
...
else if (metrics.structuralRedFlag || verdicts.some(v => v !== 'PASS')) consensus = 'CONDITIONAL';
...
const md = [
  ...
  ...reports.flatMap(r => [
    `## ${r.agent} — ${r.model}`,
    '',
    `Derived verdict: **${r.verdict}** · Highest severity: **${r.highest_severity}** · Confidence: **${r.confidence}/100** · Runtime: **${r.runtime_seconds}s**`,
    '',
    ...(r.findings.length ? r.findings.map(f => `- **${f.severity}** — ${f.evidence} → ${f.impact} → ${f.recommendation}`) : ['- No supported usability problem returned.']),
    '',
