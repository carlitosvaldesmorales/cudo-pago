const CUDO_FORMS_CONFIG = {
  QA_NOTICIAS: {env:'QA',spreadsheetId:'14ZCRIuCBtZQ_obcXzxYY3FKDScSMZG1v7UZ0954nwJI',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_NOTICIAS',title:'CUDO QA · Noticias',description:'Publica una noticia para la web CUDO. Completa sólo información que conozcas; los controles técnicos y la publicación final se gestionan automáticamente.'},
  QA_EQUIPOS: {env:'QA',spreadsheetId:'1GJYChKXx9qAwBu7fhC8V-qmoW5S1Mmq7kP8cuO6khNI',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_EQUIPOS',title:'CUDO QA · Series y Equipos',description:'Administra la información pública de una serie o equipo CUDO sin tocar planillas ni código.'},
  QA_PLANTEL: {env:'QA',spreadsheetId:'1fvJedi1WiI_lm-WFGXls4STjddAcdz3_wQN8GG11B94',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_PLANTEL',title:'CUDO QA · Jugadores / Plantel',description:'Registra información pública de un jugador. Usa fotografías autorizadas y evita datos personales que no sean necesarios para la web.'},
  QA_PARTIDOS: {env:'QA',spreadsheetId:'1AiIAh-gjtiWRTGoMAnhF-iN83XB4cWSgbeEUX_C7VbI',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_PARTIDOS',title:'CUDO QA · Fechas, Partidos y Resultados',description:'Registra una programación o resultado. Indica los datos deportivos tal como fueron informados oficialmente; la publicación final queda sujeta a control.'},
  QA_TABLA: {env:'QA',spreadsheetId:'1evGNco6Si1BYUAdwsBxLGiSMsYEmmojVWlx04NgPodY',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_TABLA',title:'CUDO QA · Tabla de Posiciones',description:'Registra una fila de la tabla oficial. No calcules datos que el sistema pueda derivar; la posición y los puntos deben coincidir con la fuente oficial mientras no estén modeladas las reglas de desempate y sanciones.'},
  QA_GALERIA: {env:'QA',spreadsheetId:'1RDs5qukBJnW8L6OBPwo4ZcB3a3xz3tI2XibceTh6Q2c',captureFolderId:'1TtGqzfFOqQTIioySe8BmX64c3wGGIRhY',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_GALERIA',title:'CUDO QA · Galería',description:'Registra una fotografía autorizada para la galería CUDO. Si aparecen menores, la publicación requiere autorización antes de hacerse pública.'},
  PROD_PARTIDOS: {env:'PROD',spreadsheetId:'1wD-UcUOU12UbCNtGyEjwXe27F3PrmzMLd6h8H3zmt68',captureFolderId:'1IQkr83hoPnoto21u0JyPcBD6mPBWZyPu',specSheet:'FORM_SPEC',rawSheet:'RAW_FORM_PARTIDOS',title:'CUDO · Partidos y Resultados',description:'Ingreso oficial de partidos y resultados CUDO. La publicación en la web requiere revisión posterior en CONTROL.'}
};
...
function provisionForm_(cfg){
  ...
  if(existingFormId){
    ...
    form.setTitle(cfg.title).setDescription(cfg.description).setConfirmationMessage('Registro recibido. Quedará pendiente de revisión antes de publicarse.').setAllowResponseEdits(false).setShowLinkToRespondAgain(true).setAcceptingResponses(true);
  ...
  const form=FormApp.create(cfg.title,true).setDescription(cfg.description).setConfirmationMessage('Registro recibido. Quedará pendiente de revisión antes de publicarse.').setAllowResponseEdits(false).setShowLinkToRespondAgain(true).setAcceptingResponses(true);
...
function reconcileQuestions_(form,specRows){
  ...
  if(items.length!==specRows.length){
    // Reemplazo controlado de preguntas, conservando FORM_ID y destino. Sólo se permite cuando
    // el contrato RAW sigue siendo Timestamp + una columna por pregunta, validado antes.
