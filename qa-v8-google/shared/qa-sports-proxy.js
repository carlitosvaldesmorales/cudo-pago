(()=>{
  ...
  const configured=String(window.CUDO_QA_SPORTS_PROXY_BASE||'').replace(/\/$/,'');
  const proxyBase=local?'http://127.0.0.1:19095':configured;
  const nativeFetch=window.fetch.bind(window);
  window.fetch=(input,init)=>{
    const raw=typeof input==='string'?input:String(input&&input.url||'');
    if(!raw.startsWith(WORKER)||!proxyBase) return nativeFetch(input,init);
