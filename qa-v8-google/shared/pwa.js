(()=>{
  if(window.CUDO_PWA&&window.CUDO_PWA.version==='2.0') return;

  const current=document.currentScript;
  const rootUrl=current?new URL('../',current.src):new URL('/qa/',location.origin);
  const state={
    version:'2.0',
  ...
  state.ready=navigator.serviceWorker
    .register('/qa/sw.js',{scope:state.scope})
    .then(registration=>{
