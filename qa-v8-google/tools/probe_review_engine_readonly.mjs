...
const noticiaRevision=await readValues(accessToken,MODULES.NOTICIA.spreadsheetId,'REVISION!A:H');
const report={
  ok:true,
  ...
  review_sheet:{
    ...
    pending_implemented_noticia:pending.filter(row=>String(row[idx.TIPO_CONTENIDO]||'').trim()==='NOTICIA').length
  },
  noticia_revision:{rows:Math.max(0,noticiaRevision.length-1),headers:noticiaRevision[0]?.map(v=>String(v||'').trim()).filter(Boolean)||[]},
  modules
};
