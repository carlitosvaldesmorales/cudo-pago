...
['G-O','2026-09-22T07:00:00Z',EVENT_IDS.MATCH,'1','MATCH_ADD_OFFERING',JSON.stringify({name:'Bebida QA',mode:'DIRECT_RESALE',sell_price:1500}),'offer','qa://offer','sistemas@cudo.cl','PENDING','',''],
['G-P','2026-09-22T07:01:00Z',EVENT_IDS.MATCH,'2','MATCH_PURCHASE',JSON.stringify({item_id:'INV-BEBIDA_QA',qty:10,unit_cost:700,supplier:'Proveedor QA',payment:'PENDING'}),'purchase','qa://purchase','sistemas@cudo.cl','PENDING','',''],
['G-S','2026-09-22T07:02:00Z',EVENT_IDS.MATCH,'3','MATCH_SALE',JSON.stringify({offering_id:'OFFER-BEBIDA_QA',qty:4,unit_price:1500,method:'CASH'}),'sale','qa://sale','sistemas@cudo.cl','PENDING','',''],
['G-BAD','2026-09-22T07:03:00Z',EVENT_IDS.BINGO,'4','BINGO_DONATE_PRIZE','{bad json','bad','qa://bad','sistemas@cudo.cl','PENDING','','']];
let n=0;const plan=planEventResourceSheetRequests({requestValues:rows,stateStore:state,registry,expectedPending:4,now:()=> '2026-09-22T07:'+String(10+n++).padStart(2,'0')+':00Z'});
assert.deepEqual(plan.summary.map(x=>x.status),['APPLIED','APPLIED','APPLIED','BLOCKED_PAYLOAD_JSON']);assert.equal(plan.state_store.store_revision,4);
...
const match=control.values.find((x,i)=>i>0&&x[1]==='MATCH');assert.equal(JSON.parse(match[5])[0].name,'Bebida QA');assert.equal(JSON.parse(match[6])[0].stock,6);
