(()=>{
const API='https://cudo-sports-event-bus.carlos-valdes-morales.workers.dev',FALLBACK='../data/championship-fixture.json',SERIES_DATA='../data/anfa-chepica-2026-series-results.json',V='20260909-logos03';
...
function crest(n){const src=ASSETS[norm(n)];return src?`<img class="champ-crest" src="${src}" alt="Escudo ${esc(n)}" loading="lazy" decoding="async">`:'<span class="champ-crest champ-crest-fallback" aria-hidden="true">⚽</span>'}
...
function render(d){
 ...
 const draw=()=>{
   ...
   document.getElementById('champPublished').textContent=round===1?'Resultados oficiales disponibles por serie · 23 agosto 2026':'Fixture programado · resultados pendientes';
 };
 ...
 document.getElementById('champCount').textContent=`${d.matches.length} partidos · ${d.results.length} con resultados oficiales`;
 document.getElementById('champLive').textContent=d.mode==='live'?'Fixture conectado al Sports Event Bus':'Fixture oficial publicado · sincronización segura';
 draw();
}
