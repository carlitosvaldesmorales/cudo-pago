...
function build(fixture,resultDoc,rules,group){
  ...
  for(const match of resultDoc.results||[]){
    ...
    const seriesMap=new Map((match.series||[]).map(s=>[s.series,s]));
...
async function boot(){
  ...
  try{
    const [f,r,ru]=await Promise.all([fetch(`${FIXTURE}?v=${V}`,{cache:'no-store'}),fetch(`${RESULTS}?v=${V}`,{cache:'no-store'}),fetch(`${RULES}?v=${V}`,{cache:'no-store'})]);
    const fixture=await f.json(),results=await r.json(),rules=await ru.json();
    let group='A';
