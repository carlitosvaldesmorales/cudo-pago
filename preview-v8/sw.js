const CACHE_NAME='cudo-mobile-shell-v1';
const SCOPE='/preview-v8/';
...
self.addEventListener('activate',event=>{
  event.waitUntil(
    caches.keys()
      .then(keys=>Promise.all(keys.filter(key=>key.startsWith('cudo-mobile-shell-')&&key!==CACHE_NAME).map(key=>caches.delete(key))))
