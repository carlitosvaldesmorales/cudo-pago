...
globalThis.fetch=async(url,init={})=>{
  ...
  return new Response(JSON.stringify({ok:true,result:{message_id:body.message_id||55}}),{status:200,headers:{'content-type':'application/json'}});
};
...
async function cb(actor,data,messageId=55){return dispatch({callback_query:{id:`cb-${updateId+1}`,from:actor,data,message:{message_id:messageId,chat:{id:actor.id,type:'private'}}}});}
async function msg(actor,text){return dispatch({message:{message_id:10,from:actor,text,chat:{id:actor.id,type:'private'}}});}
...
async function guided(actor,match,series,home,away){
  await cb(actor,'rr:dates');
  await cb(actor,`rr:date:${match.round_no}`);
  let r=await cb(actor,`rr:match:${match.match_id}`);assert.equal(r.handled,'results_register_match_dashboard');
  await cb(actor,`rr:series:${match.match_id}:${series}`);
  const n=await nonce(actor);
  await cb(actor,`rr:h:${n}:${home}`);
  await cb(actor,`rr:a:${n}:${away}`);
  r=await cb(actor,`rr:confirm:${n}`);
  assert.equal(r.handled,'results_register_applied');
  const lastEdit=[...calls].reverse().find(x=>x.method==='editMessageText');
  assert.match(String(lastEdit?.body?.text||''),/RESULTADOS DEL PARTIDO/);
  return r;
}
function assertTelegramContract(){
  const screens=calls.filter(x=>['sendMessage','editMessageText'].includes(x.method));
  for(const call of screens){
  ...
  const text=screens.map(x=>String(x.body.text||'')).join('\n');
  assert.doesNotMatch(text,/Escribe el marcador|LOCAL-VISITA/i);
  assert.match(text,/CONFIRMAR SERIE/);
  assert.match(text,/RESULTADOS DEL PARTIDO/);
  assert.ok(calls.some(x=>x.method==='editMessageText'),'Golden Path debe editar una superficie viva');
}

try{
  applyMigrations();

  let r=await msg(ACTOR.PUBLIC,'/registrarresultado');
  assert.equal(r.handled,'results_register_dates');
  assert.equal(calls.filter(x=>x.method==='sendMessage').length,1);
  console.log('PASS command entry creates one Telegram live surface');
  ...
  const beforeGuided=calls.length;
  r=await guided(ACTOR.PUBLIC,publicMatch,'TERCERA',2,1);
  assert.equal(r.outcome,'SUBMITTED');
  assert.equal(calls.slice(beforeGuided).filter(x=>x.method==='sendMessage').length,0,'Callback Golden Path no debe acumular mensajes');
  assert.ok(calls.slice(beforeGuided).filter(x=>x.method==='editMessageText').length>=6,'Golden Path debe reutilizar superficie');
  ...
  const dashboard=[...calls].reverse().find(x=>x.method==='editMessageText');
  assert.match(dashboard.body.text,/🕒 3ª\s+2 — 1\s+INFORMADO/);
  assert.match(dashboard.body.text,/1\/4 con dato/);
  ...
  console.log('PASS match work unit -> series persistence -> SUBMITTED -> dashboard -> push snapshot');
  ...
  await cb(ACTOR.PUBLIC,`rr:series:${publicMatch.match_id}:SEGUNDA`);const staleNonce=await nonce(ACTOR.PUBLIC);
  r=await cb(ACTOR.PUBLIC,`rr:cancel:${staleNonce}`);assert.equal(r.handled,'results_register_series_cancel');
  const afterCancel=[...calls].reverse().find(x=>x.method==='editMessageText');assert.match(afterCancel.body.text,/Serie cancelada/);assert.match(afterCancel.body.text,/🕒 3ª\s+2 — 1/);
  r=await cb(ACTOR.PUBLIC,`rr:h:${staleNonce}:2`);assert.equal(r.handled,'results_register_stale');assert.equal(await one('SELECT nonce FROM telegram_result_register_sessions WHERE telegram_user_id=?',String(ACTOR.PUBLIC.id)),null);
  console.log('PASS cancel returns to match dashboard, preserves saved series, stale callback fails closed');
  ...
  const officialDashboard=[...calls].reverse().find(x=>x.method==='editMessageText');assert.match(officialDashboard.body.text,/✅ 1ª\s+1 — 0\s+OFICIAL/);
  console.log('PASS club admin canonical capture -> official series -> same match dashboard + immutable baseline + push');

  const other={id:9931003,first_name:'Segundo'};
  const otherMatch=round3.find(m=>m.match_id!==publicMatch.match_id);assert.ok(otherMatch);
  await cb(ACTOR.PUBLIC,`rr:series:${publicMatch.match_id}:SEGUNDA`);const n1=await nonce(ACTOR.PUBLIC);
  await cb(other,`rr:series:${otherMatch.match_id}:SENIOR`);const n2=await nonce(other);
  assert.notEqual(n1,n2);
  const s1=await one('SELECT match_id,series_code FROM telegram_result_register_sessions WHERE telegram_user_id=?',String(ACTOR.PUBLIC.id));
  const s2=await one('SELECT match_id,series_code FROM telegram_result_register_sessions WHERE telegram_user_id=?',String(other.id));
  assert.equal(s1.match_id,publicMatch.match_id);assert.equal(s2.match_id,otherMatch.match_id);
  console.log('PASS concurrent identities keep independent match/series sessions');
  ...
  console.log('PASS RESULTS-REGISTER v2 match-centered single-surface contract');
