...
import { CAPABILITY, hasCapability, hasScopedCapability } from '../../sports-bus/worker/access-control.js';
...
try{
  ...
  let r=await guided(ACTOR.PUBLIC,missing,'TERCERA',2,1);
  assert.equal(r.source_type,'PUBLIC_USER');
  ...
  console.log('PASS public observation never mutates canonical state');

  await seed(ACTOR.MEDIA,'REPORTER',null,'PROVISIONAL');
  const now=new Date().toISOString();
  await env.DB.prepare(`INSERT INTO actor_scope_grants (grant_id,telegram_user_id,role,scope_type,scope_id,capabilities_json,trust_level,source_label,granted_by,active,created_at,updated_at,partner_code) VALUES (?,?,?,?,?,?,?,?,?,1,?,?,?)`)
    .bind('qa-media-competition',String(ACTOR.MEDIA.id),'MEDIA_PARTNER','COMPETITION','ANFA-CHEPICA-2026',JSON.stringify(['READ_COMPETITION','OBSERVE_RESULT']),'VERIFIED','Chépica Play','qa-super',now,now,'CHEPICA_PLAY').run();
  const mediaReporter=await one('SELECT * FROM reporters WHERE telegram_user_id=?',String(ACTOR.MEDIA.id));
  assert.equal(await hasScopedCapability(env.DB,mediaReporter,CAPABILITY.READ_COMPETITION,official),true);
  assert.equal(await hasScopedCapability(env.DB,mediaReporter,CAPABILITY.OBSERVE_RESULT,official),true);
  assert.equal(await hasScopedCapability(env.DB,mediaReporter,CAPABILITY.PUBLISH_MATCH_EVENT,official),false);

  let h=Number(official.official_home),a=Number(official.official_away);
  if(h<7) h+=1; else if(a<7) a+=1; else h=0;
  r=await guided(ACTOR.MEDIA,official,official.series_code,h,a);
  assert.equal(r.source_type,'MEDIA_PARTNER');
  const mediaObs=await one('SELECT * FROM public_result_submissions WHERE submission_id=?',r.submission_id);
  assert.equal(mediaObs.source_label,'Chépica Play');
  assert.equal(mediaObs.trust_level,'VERIFIED');
  const still=await one('SELECT home_score,away_score,validation_status FROM match_series_results WHERE result_id=?',official.result_id);
  assert.equal(Number(still.home_score),Number(official.official_home));
  assert.equal(Number(still.away_score),Number(official.official_away));
  assert.equal(still.validation_status,official.official_status);
  console.log('PASS Chépica Play registers results competition-wide without coverage and preserves canonical truth');
  ...
  console.log('PASS operational authority remains separated from policy authority');

  await seed(ACTOR.OPERATOR,'PLATFORM_OPERATOR',null,'VERIFIED');
  const second=(await all(`SELECT m.* FROM matches m WHERE m.competition_id='ANFA-CHEPICA-2026' AND m.match_id<>? ORDER BY m.round_no,m.match_id LIMIT 1`,missing.match_id))[0];
  r=await guided(ACTOR.OPERATOR,second,'SEGUNDA',1,0);
  assert.equal(r.source_type,'PLATFORM_OPERATOR');
