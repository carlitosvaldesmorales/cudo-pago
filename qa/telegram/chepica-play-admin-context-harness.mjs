...
try{
  ...
  assert.equal(await getTelegramEntryContext(DB,ADMIN.id),'CHEPICA_PLAY');
  console.log('PASS SUPER_ADMIN enters Chépica Play context and is routed to existing observation capability');

  const grantCount=await DB.prepare("SELECT COUNT(*) AS n FROM actor_scope_grants WHERE telegram_user_id=? AND partner_code='CHEPICA_PLAY'")
    .bind(String(ADMIN.id)).first();
  assert.equal(Number(grantCount.n),0);
  ...
  console.log('PASS context switch changes neither actor identity nor authorization');
  ...
  await DB.prepare(`INSERT INTO public_result_submissions
    ...
    VALUES ('qa-admin-context','A-F3-M1','TERCERA',?,?,1,0,'SUBMITTED','telegram','qa-event',?,?)`)
    .bind(String(ADMIN.id),'Admin Global',now,now).run();

  const submission=await DB.prepare(`SELECT submitter_id,submitter_name,source_channel,entry_context
    FROM public_result_submissions WHERE submission_id='qa-admin-context'`).first();
  assert.equal(submission.submitter_id,String(ADMIN.id));
  assert.equal(submission.submitter_name,'Admin Global');
  assert.equal(submission.entry_context,'CHEPICA_PLAY');
  assert.equal(submission.source_channel,'telegram:chepica_play');
  console.log('PASS result audit stores real actor + Chépica Play entry context separately');
  ...
  prepared=await prepareTelegramEntryContext(await canonicalCallback(ADMIN,'tp:public-report'),env);
  assert.equal(prepared.response,null);
